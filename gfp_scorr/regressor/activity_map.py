...
def activity_map(eeg_data, spike_timing):
    """
    Construct the epileptic map by averaging EEG data at spike times
    and generate a topographic map.
    
    Parameters:
        eeg_data: ndarray
            Preprocessed EEG data array with shape (n_channels, n_samples).
        spike_timing: ndarray
            Array of spike peak indices.
    
    Returns:
        epileptic_map: ndarray
            Normalized epileptic map (mean spike form).
    """
    spike_data = []
    window = 50  # Number of samples to include before and after the spike
